...
namespace MarsRoverSimulator.UserInterface
{
	public class UI
	{
		...
		public string SetMovementPlan(int serial)
		{
			var response =
				IO.GetUserResponse(
					$"Enter the movement plan for rover #{serial} (L: left  R:right M:move -  for example LMLMLMLMM )");

			//the only valid response contains L M R, so lets reject everything but that.
			var strPattern = new Regex("^[lrmLRM]");

			if (!strPattern.IsMatch(response))
			{
				IO.SendTextToUser(
					"Invalid parameters.  Please use this format (L: left  R:right M:move -  for example LMLMLMLMM )");
